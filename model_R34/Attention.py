...
class RTrans(nn.Module):
    def __init__(self, d_model, h=4):
        ...
        self.d_k = d_model//h
        self.h = h
        self.query = nn.Conv2d(self.d_model, self.d_model, 1, bias=False)
        self.key = nn.Conv2d(self.d_model, self.d_model, 1, bias=False)
        self.value = nn.Conv2d(self.d_model, self.d_model, 1, bias=False)
        self.softmax = nn.Softmax(-2)

    def self_attention(self, q, k, v):
        attention = self.softmax(torch.matmul(k.transpose(-1, -2).contiguous(), q)/math.sqrt(self.d_k))
        v = torch.matmul(v, attention)
        return v

    def forward(self, q, k, v):
        B, C, H, W = q.size()
        q, k, v = self.query(q).view(B, self.d_model, H*W).view(B, self.h, self.d_k, H*W),\
...
class OldRTrans(nn.Module):
    def __init__(self, d_model, h=2):
        ...
        self.d_k = d_model//h
        self.h = h
        self.query = nn.Conv2d(self.d_model, self.d_k, 1)
    ...
    def forward(self, q, k, v):
        ...
        B, C, H, W = q.size()
        q, k, v = self.query(q).view(B, self.d_k, H*W).transpose(1, 2).contiguous(),\
            self.key(k).view(B, self.d_k, H*W),\
            self.value(v).view(B, self.d_model, H*W)
