...
class Interactive(nn.Module):
    def __init__(self, spatial_ckpt=None, temporal_ckpt=None):
        ...
        self.TTTransformer1 = RTModule(256, h=1, ST=False, redunction=True)
        self.TTTransformer2 = RTModule(512, h=2, ST=False)
        self.TTTransformer3 = RTModule(1024, h=4, ST=False)
        self.TTTransformer4 = RTModule(2048, h=8, ST=False)
        self.TselfTransformer1 = SelfModule(256, h=head*1, redunction=True)  # kv=T q=S
        self.TselfTransformer2 = SelfModule(512, h=head*2)
        self.TselfTransformer3 = SelfModule(1024, h=head*4)
        self.TselfTransformer4 = SelfModule(2048, h=head*8)
    ...
    def spatial_decoder(self, x, spatial_f, temporal_f):
        ...
        feature3 = self.spatial_net.decoder3(
            torch.cat((F.interpolate(feature4, (H, W), mode='bilinear', align_corners=True),
                       self.Transformerstafm3(self.spatial_net.bridge3(spatial_f[2]),
                                              self.temporal_net.bridge3(temporal_f[2]))), 1))
        ...
        feature2 = self.spatial_net.decoder2(
            torch.cat((F.interpolate(feature3, (H, W), mode='bilinear', align_corners=True),
                       self.Transformerstafm2(self.spatial_net.bridge2(spatial_f[1]),
                                              self.temporal_net.bridge2(temporal_f[1]))), 1))
        ...
        feature1 = self.spatial_net.decoder1(
            torch.cat((F.interpolate(feature2, (H, W), mode='bilinear', align_corners=True),
                       self.Transformerstafm1(self.spatial_net.bridge1(spatial_f[0]),
                                              self.temporal_net.bridge1(temporal_f[0]))), 1))
    ...
    def temporal_decoder(self, x, f):
        ...
        B, C, H, W = f[2].size()
        feature3 = self.temporal_net.decoder3(torch.cat((
            F.interpolate(feature4, (H, W), mode='bilinear', align_corners=True), self.temporal_net.bridge3(f[2])), 1))
        B, C, H, W = f[1].size()
        feature2 = self.temporal_net.decoder2(torch.cat((
            F.interpolate(feature3, (H, W), mode='bilinear', align_corners=True), self.temporal_net.bridge2(f[1])), 1))
        B, C, H, W = f[0].size()
        feature1 = self.temporal_net.decoder1(torch.cat((
            F.interpolate(feature2, (H, W), mode='bilinear', align_corners=True), self.temporal_net.bridge1(f[0])), 1))
