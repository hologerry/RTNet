...
class Temporal(nn.Module):
    def __init__(self):
        ...
        # -------------Encoder--------------
        self.inconv = nn.Sequential(nn.Conv2d(6, 64, 7, stride=2, padding=3, bias=False), resnet.bn1, resnet.relu, resnet.maxpool)
        self.encoder1 = resnet.layer1  # 256
        self.bridge1 = nn.Sequential(nn.Conv2d(256, 64, 1, bias=False), nn.BatchNorm2d(64), nn.ReLU(True))
        self.encoder2 = resnet.layer2  # 128
        self.bridge2 = nn.Sequential(nn.Conv2d(512, 128, 1, bias=False), nn.BatchNorm2d(128), nn.ReLU(True))
        self.encoder3 = resnet.layer3  # 64
        self.bridge3 = nn.Sequential(nn.Conv2d(1024, 256, 1, bias=False), nn.BatchNorm2d(256), nn.ReLU(True))
        self.encoder4 = resnet.layer4  # 32
        # ------------dilation---------------#
        self.dilation0 = nn.Sequential(nn.Conv2d(2048, 512, 1, bias=False), nn.BatchNorm2d(512), nn.ReLU(True))
        self.dilation1 = nn.Sequential(nn.Conv2d(2048, 512, 3, dilation=1, padding=1, bias=False), nn.BatchNorm2d(512), nn.ReLU(True))
        self.dilation2 = nn.Sequential(nn.Conv2d(2048, 512, 3, dilation=2, padding=2, bias=False), nn.BatchNorm2d(512), nn.ReLU(True))
        self.dilation3 = nn.Sequential(nn.Conv2d(2048, 512, 3, dilation=4, padding=4, bias=False), nn.BatchNorm2d(512), nn.ReLU(True))
        self.dilation4 = nn.Sequential(nn.Conv2d(2048, 512, 3, dilation=8, padding=8, bias=False), nn.BatchNorm2d(512), nn.ReLU(True))
        # ------------decoder----------------#
    ...
    def encoder(self, x):
        ...
        h5 = torch.cat([self.dilation0(h4), self.dilation1(h4), self.dilation2(h4), self.dilation3(h4), self.dilation4(h4)], 1)
        return [h1, h2, h3, h4], h5

    def decoder(self, x, f):
        feature4 = self.decoder4(x)
        B, C, H, W = f[2].size()
        feature3 = self.decoder3(torch.cat((F.interpolate(feature4, (H, W), mode='bilinear', align_corners=True), self.bridge3(f[2])), 1))
        B, C, H, W = f[1].size()
        feature2 = self.decoder2(torch.cat((F.interpolate(feature3, (H, W), mode='bilinear', align_corners=True), self.bridge2(f[1])), 1))
        B, C, H, W = f[0].size()
        feature1 = self.decoder1(torch.cat((F.interpolate(feature2, (H, W), mode='bilinear', align_corners=True), self.bridge1(f[0])), 1))
